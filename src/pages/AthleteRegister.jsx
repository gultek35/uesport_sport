...
export default function AthleteRegister() {
  ...
  useEffect(() => {
    ...
    // Gerçek doldurulmuş alan sayısını hesapla
    let filled = 0;
    ...
    // Bölüm bazında doldurulma kontrolü
    const completed = sections.filter((s) => {
      const total = parseInt(s.count.split(" / ")[1]) || 0;
      const filledInSection = Object.keys(formData).filter(key => {
        return formData[key] && formData[key] !== "" && formData[key] !== null;
      }).length;
      return filledInSection >= total;
    }).length;

    const percent = totalFields > 0 ? Math.round((filled / totalFields) * 100) : 0;
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      setMessage({
        type: "error",
        text: "❌ Taslak kaydedilemedi: " + (err.response?.data?.detail || err.message),
      });
    }
  };

  // 📌 KAYDET VE YENİ SPORCU EKLE
      ...
      <div className="reg-shell">
        ...
        <main className="reg-content">
          ...
          <section className="reg-progress-card">
            ...
            <div className="reg-progress-right">
              <strong>
                {progress.completedSections} bölümden {sections.length}'i tamamlandı
              </strong>
        ...
        <footer className="reg-bottom-actions">
          ...
          <div>
            ...
            <button type="button" className="secondary" onClick={handleSaveAndNew}>
              ♙ Kaydet ve Yeni Sporcu Ekle
            </button>
            <button type="submit" className="primary" onClick={handleSaveAndContinue}>
              Kaydet ve İlerle →
            </button>
          </div>
        </footer>
